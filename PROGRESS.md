...
最終更新: 2026-09-25(追加機能F1〜F3の実装・ローカル検証)
...
2026-09-25に[追加機能調査・作業計画](追加機能調査_作業計画_2026-09-25.md)を作成し、F1〜F3を実装した。開発中のGitHub Actionsは禁止、最終公開時のGitHub Pagesのみ許可。
...
## 追加機能実装と確認(2026-09-25)

- **M0:** GitHubリポジトリ設定でActionsを無効化し、再読込後も「Disable actions」であることを確認。既存Pagesサイトを一時的にUnpublishし、作業ブランチから `.github/workflows/ci.yml` と `deploy-pages.yml` を撤去した。最終Pages公開まではActionsを再有効化しない。
- **M1:** [フェーズ0環境確認](docs/phase0/2026-09-25-environment.md)を記録。CMakeは利用できるがEmscripten/Android接続環境と実撮影データ・CAD正解形状が不足し、0-3〜0-8の合否は未判定。F4/F5は実データで測定するまで着手判断を保留。
- **M2/F1:** 採用・除外、解像度、ブレ候補、焦点距離候補欠損、機種混在を表示。サムネイル由来の露出/近似類似候補を画像カードに表示。画像詳細で焦点距離px値と入力根拠を保存可能。診断で自動除外しない。
- **M3/F2:** PLY点群/三角面、ASCII/binary STLをWorkerで解析。有限座標・面索引・容量を検査し、入力単位からプロジェクト単位へ換算して段階・アセット・Blobを単一transactionで追記。履歴/ビューア/出力に外部取込由来を表示。
...
1. 追加機能のPRをレビューし、指摘を解消してマージする。開発中のActions無効化は維持する。
2. 最終Pages公開はローカルビルド成果物を手動で配信用ブランチへ置き、Pagesのブランチ公開を設定する。Pages内部実行以外のワークフローは作らない。
3. Emscripten固定版、20〜60枚の実撮影画像、CAD正解形状、Android端末を揃え、フェーズ0の技術・性能・精度検証を続ける。F4/F5はこの測定で採否を決める。

## 追加機能の再調査(2026-09-25)

Web、GitHubの一次資料と現在のコードを照合し、画像セット診断、外部PLY/STL取込、大容量ZIPの逐次入出力を優先候補とした。表示専用軽量化とOPFS保存は実データの計測後に採否を決める。調査根拠、実装箇所、受入条件は[追加機能調査_作業計画_2026-09-25.md](追加機能調査_作業計画_2026-09-25.md)に記載。今回は計画文書だけを作成し、アプリコード、ワークフロー、リモート設定は変更していない。
ローカル参照先を検査し、`npm ci --offline` 後の `npm run build`（ライセンス検査、型検査、本番ビルド、PWA資源生成）と `git diff --check` が成功した。
