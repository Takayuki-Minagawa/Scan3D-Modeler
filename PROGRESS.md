...
最終更新: 2026-09-25(追加機能PRのマージ・計画文書の整理)
...
| 計画項目 | 状態 | 実装場所/備考 |
|---|---|---|
| 1A-1 雛形+COOP/COEP | ✅ | `app/vite.config.ts`。開発用CIはActions利用制限により撤去済み |
...
- [x] 大容量ZIPのメモリ使用(fflateを全メモリで使用) → 出力を逐次化し、読込を容量制限付きで逐次展開
...
1. 最終Pages公開はローカルビルド成果物を手動で配信用ブランチへ置き、Pagesのブランチ公開を設定する。Pages内部実行以外のワークフローは作らない。
2. Emscripten固定版、20〜60枚の実撮影画像、CAD正解形状、Android端末を揃え、フェーズ0の技術・性能・精度検証を続ける。F4/F5はこの測定で採否を決める。

## 追加機能の再調査(2026-09-25)

Web、GitHubの一次資料と現在のコードを照合し、画像セット診断、外部PLY/STL取込、大容量ZIPの逐次入出力を優先して実装した。表示専用軽量化と永続的なOPFS保存は、実データの速度・メモリ・保存時間を計測した後に採否を決める。調査時の詳細な計画はGit履歴 `fdedfdc` に残す。
