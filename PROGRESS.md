...
最終更新: 2026-09-25(追加機能のWeb・GitHub調査と作業計画)
...
2026-09-25に[追加機能調査・作業計画](追加機能調査_作業計画_2026-09-25.md)を作成。新機能は未実装。開発中のGitHub Actionsは禁止、最終公開時のGitHub Pagesのみ許可という運用条件を記録した。
...
1. **次のpush/PRより前にGitHub Actionsを停止**: 既存のCI・自動Pages公開ワークフローをリポジトリ設定で止め、ワークフローファイルを撤去する。最終GitHub Pages公開は例外として[追加機能調査・作業計画](追加機能調査_作業計画_2026-09-25.md)に手順を分離。現時点では停止・撤去は未実施
2. **フェーズ0の技術検証を継続**: 固定Emscripten/CMake環境でOpenMVG最小ターゲットのWASMビルド、Worker実行、メモリ、IndexedDB保存を検証
3. 自前MVSの最小品質試験とPoissonRecon→Manifold→fTetWildの貫通試験。記録は`docs/phase0/`へ追記
4. 検証用の実撮影データ(使用書§30の対象物)とAndroid/実カメラで1B・0-7・0-8を確認
5. [追加機能調査・作業計画](追加機能調査_作業計画_2026-09-25.md)のF1〜F3を進める。F4/F5、プロジェクト複製、スクリーンショットは実データの規模と端末性能を見て採否を決める
